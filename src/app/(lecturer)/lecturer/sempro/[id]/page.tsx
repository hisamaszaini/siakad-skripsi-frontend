...
import { ChevronLeft, ClipboardCheck, AlertCircle, Loader2, User, Calendar, Book, FileText, ExternalLink, IdCard } from "lucide-react";
import { Input } from "@/components/ui/input";
import { useSemproByIdQuery, useSemproEvaluation } from "@/features/sempro";
...
const SEMPRO_STATUS_OPTIONS = [
    { value: "PASSED", label: "LULUS (DITERIMA)" },
    { value: "REVISE", label: "LULUS DENGAN REVISI" },
    { value: "FAILED", label: "TIDAK LULUS (DITOLAK)" }
];
...
export default function LecturerSemproResultPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const { form, onSubmit, isPending } = useSemproEvaluation(id, sempro);
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                <div>
                    ...
                    <h1 className="text-4xl font-extrabold tracking-tight text-slate-900 leading-none">
                        Berita <span className="text-indigo-600">Acara</span>
                    </h1>
                </div>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-4 pt-4 border-t border-slate-50">
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div>
                                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Judul Proposal</p>
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div>
                                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Waktu Seminar</p>
                                ...
                                <div className="pt-4 border-t border-slate-50">
                                    ...
                                    >
                                        ...
                                        <span className="text-sm">Preview Proposal</span>
                            ...
                            <Card className="border-none shadow-xl shadow-slate-200/50 bg-white rounded-[2.5rem] overflow-hidden pt-0 gap-0 ring-0">
                                <div className="h-40 bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 relative rounded-t-[2.5rem]">
                                    <div className="absolute bottom-10 left-10">
                                        <h2 className="text-3xl font-black text-white tracking-tight">Lembar Penilaian</h2>
                                ...
                                <CardContent className="p-10 space-y-10">
                                    <div className="grid md:grid-cols-2 gap-8">
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <FormItem className="space-y-3">
                                                    <FormLabel className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Keputusan Akhir</FormLabel>
                                                    <FormControl>
                                                        <SingleSelect
                                                            value={(field.value as string) === 'SCHEDULED' || !field.value ? undefined : field.value}
                                                            onChange={field.onChange}
                                                            options={SEMPRO_STATUS_OPTIONS}
                                                            placeholder="Pilih Status..."
                                                            showSearch={false}
                                                            className="h-14 w-full rounded-[1.5rem] border-slate-100 bg-slate-50/50 px-8 font-black text-md focus-visible:ring-indigo-600/20 hover:border-indigo-400 hover:bg-white transition-all text-slate-900 shadow-sm shadow-indigo-100/20 justify-between items-center"
                                                        />
                                                    </FormControl>
                                                    <FormMessage />
                                                </FormItem>
                                        ...
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <FormItem className="space-y-3">
                                                    <FormLabel className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Nilai Angka (0-100)</FormLabel>
                                                    <FormControl>
                                                        <Input
                                                            type="number"
                                                            placeholder="85"
                                                            className="h-14 rounded-[1.5rem] border-slate-100 bg-slate-50/50 px-8 font-extrabold text-md focus-visible:ring-indigo-600/20 focus-visible:border-indigo-600 hover:border-indigo-400 hover:bg-white transition-all shadow-sm shadow-indigo-100/20"
                                                            {...field}
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem className="space-y-3">
                                                <FormLabel className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Catatan Revisi / Masukan</FormLabel>
                                                <FormControl>
                                                    <Textarea
                                                        placeholder="Tuliskan poin-poin revisi yang harus diperbaiki oleh mahasiswa..."
                                                        className="min-h-[220px] rounded-[2rem] border-slate-100 bg-slate-50/50 p-8 font-medium focus-visible:ring-indigo-600/20 focus-visible:border-indigo-600 hover:border-indigo-400 hover:bg-white transition-all leading-relaxed shadow-sm shadow-indigo-100/20"
                                                        {...field}
                                                    />
                                                </FormControl>
                                                <FormMessage />
                                ...
                                <div className="p-10 pt-0">
                                    <Button
                                        ...
                                        disabled={isPending}
                                    >
                                        {isPending ? <Loader2 className="h-6 w-6 animate-spin" /> : <ClipboardCheck className="h-6 w-6" />}
                                        Simpan Berita Acara
                                    </Button>
                                </div>
                            </Card>
