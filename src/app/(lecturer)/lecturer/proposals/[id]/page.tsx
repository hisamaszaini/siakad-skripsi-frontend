...
import { proposalService } from "@/features/proposal";
...
import {
    ChevronLeft, CheckCircle, XCircle, RefreshCcw, Edit3, Loader2,
    GraduationCap, User, Hash, BookOpen, Clock, AlertCircle, LucideIcon
} from "lucide-react";
...
export default function VerificationDetailPage() {
    ...
    const { data, error, isLoading, mutate } = useSWR(
        ...
        () => proposalService.getProposalById(id).then((res) => {
            const proposalData = res.data.data;
            if (proposalData && !editedJudul) {
                setEditedJudul(proposalData.judul);
            }
            return proposalData;
        })
    );
    ...
    const handleAction = async (action: 'APPROVE' | 'REJECT' | 'REVISE' | 'EDIT') => {
        if (!proposal) return;
        
        if ((action === 'REJECT' || action === 'REVISE') && !catatan.trim()) {
            return toast.error("Alasan wajib diisi untuk penolakan atau revisi.");
        }
        if (action === 'EDIT' && (!editedJudul.trim() || editedJudul === proposal.judul)) {
            if (!editedJudul.trim()) return toast.error("Judul baru tidak boleh kosong.");
        ...
        try {
            await proposalService.verifyProposal(id, {
                status: statusMap[action],
                catatan: catatan || undefined,
                judul: editedJudul !== proposal.judul ? editedJudul : undefined,
            });
// ... (omitting lines between 97 and 294 for brevity within the tool's chunk if needed, but I'll provide full replacement content for the chunk)
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    {editingTitle ? (
                                        <div className="flex gap-2 items-center">
                                            <Input
                                                ...
                                                autoFocus
                                            />
                                            <Button size="icon" variant="ghost" onClick={() => setEditingTitle(false)} className="shrink-0 text-slate-400 rounded-xl h-12 w-12 hover:bg-slate-100">
                                                <XCircle className="h-5 w-5" />
                                            </Button>
                                        </div>
                                    ) : (
                                        <div className="flex items-start gap-2 group relative">
                                            <CardTitle className="text-xl font-black text-slate-900 leading-tight group-hover:text-amber-600 transition-colors cursor-pointer pr-8" onClick={() => { if (canAct) { setEditingTitle(true); setEditedJudul(proposal.judul); } }}>
                                                &quot;{proposal.judul}&quot;
                                            </CardTitle>
                                            {canAct && (
                                                <Button
                                                    size="icon"
                                                    variant="ghost"
                                                    onClick={() => { setEditingTitle(true); setEditedJudul(proposal.judul); }}
                                    ...
                                    <div className="flex items-center gap-3 mt-4 text-slate-400">
                                        <div className="flex items-center gap-1.5 font-bold text-[10px] uppercase tracking-wider">
                                            ...
                                            Diajukan {format(new Date(proposal.created_at), "dd MMMM yyyy", { locale: idLocale })}
                                        </div>
                                    </div>
                                </div>
                        ...
                        <Card className="border-none shadow-sm bg-white rounded-3xl overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-700 delay-300">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex gap-3 mt-4">
                                                <Button
                                                    ...
                                                    disabled={loading || editedJudul === proposal.judul}
                                                ...
                                                >
                                                    {loading && activeAction === 'EDIT' ? <Loader2 className="h-3 w-3 animate-spin mr-1.5" /> : null}
                                                    Simpan Judul Saja
                                                </Button>
                                                <Button variant="ghost" size="sm" onClick={() => { setEditingTitle(false); setEditedJudul(proposal.judul); }}
                            ...
                            {!editingTitle && (
                                <CardFooter className="flex flex-wrap gap-4 p-8 pt-0">
                                    ...
                                    >
                                        {loading && activeAction === 'APPROVE' ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <CheckCircle className="h-4 w-4 mr-2" />}
                                        Setujui Usulan
                                    </Button>
                                    <div className="flex gap-3 w-full sm:w-auto flex-1">
                                        ...
                                        >
                                            {loading && activeAction === 'REVISE' ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <RefreshCcw className="h-4 w-4 mr-2" />}
                                            Revisi
                                        </Button>
                                        <Button
                                            variant="outline"
                                        ...
                                        >
                                            {loading && activeAction === 'REJECT' ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <XCircle className="h-4 w-4 mr-2" />}
                                            Tolak
                                        </Button>
                                    </div>
                                </CardFooter>
                            ...
                            <div className="space-y-4 pt-6 border-t border-white/10">
                                ...
                                <div className="flex items-center justify-between group cursor-help">
                                    ...
                                    <span className="text-sm font-black text-right">
                                        {format(new Date(proposal.created_at), "dd MMM yy", { locale: idLocale })}
                                    </span>
                                </div>
                            </div>
                        </CardContent>
