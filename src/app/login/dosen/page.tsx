...
export default function LecturerLoginPage() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        if (!userId || !password) {
            toast.error("NIDN/NIK dan Password wajib diisi");
            return;
        }

        setLoading(true);
        try {
            const result = await loginAction({ role: "LECTURER", userId, password });
            ...
            toast.success("Login Dosen berhasil!");

            if (result.user) {
                localStorage.setItem("user_data", JSON.stringify(result.user));
            }

            router.push("/lecturer/guidance");
    ...
    return (
        <div className="relative min-h-screen w-full flex items-center justify-center bg-white overflow-hidden font-sans">
            <PageTitle title="Login Dosen" />

            {/* Premium Background Decorations */}
            <div className="absolute inset-0 pointer-events-none">
                <div className="absolute top-[-10%] left-[-5%] w-[50%] h-[50%] bg-blue-600/5 rounded-full blur-[120px] animate-pulse" />
                <div className="absolute bottom-[-10%] right-[-5%] w-[40%] h-[40%] bg-blue-600/5 rounded-full blur-[120px] animate-pulse delay-1000" />
            </div>
            ...
            <div className="container relative z-10 px-4 flex flex-col items-center">
                ...
                <div className="w-full max-w-[600px] animate-in fade-in zoom-in-95 duration-700">
                    <div className="bg-white rounded-[2.5rem] p-12 shadow-[0_32px_64px_-12px_rgba(0,0,0,0.08)] border border-slate-200/60 backdrop-blur-sm relative overflow-hidden group">
                        ...
                        <div className="relative z-10 space-y-10">
                            ...
                            <div className="space-y-4 text-center">
                                <div className="mx-auto h-16 w-16 bg-blue-50 rounded-2xl flex items-center justify-center text-blue-600 shadow-inner group-hover:rotate-12 transition-transform duration-500">
                                    <UserCheck className="h-8 w-8" />
                                </div>
                                <div className="space-y-2">
                                    <p className="text-[10px] font-black text-blue-600 uppercase tracking-[0.4em]">Portal Dosen & Pembimbing</p>
                                    <h1 className="text-3xl font-black text-slate-900 tracking-tight leading-none italic">LECTURER LOGIN.</h1>
                                </div>
                            </div>
                            ...
                            <p className="text-center text-[9px] font-bold text-slate-300 uppercase tracking-widest leading-relaxed">
                                SIAKAD SYSTEM • EXPERT GUIDANCE PORTAL<br />
